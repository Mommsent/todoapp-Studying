...
type CreateTaskRequest struct {
	...
	Description *string `json:"description" validate:"omitempty,min=1,max=1000" examle:"Сделать до четверга домашнее задание"`
	AuthorID    int     `json:"author_user_id" validate:"required"              example:"5"`
}
...
// @Failuer     404 {object} core_http_response.ErrorResponse "Author not found"
...
func (h *TasksHTTPHandler) CreateTask(rw http.ResponseWriter, r *http.Request) {
	...
	taskDomain := domain.NewTaskUninitiolized(
		request.Title,
		request.Description,
		request.AuthorID,
	)
