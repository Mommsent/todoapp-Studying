...
type GetStatisticsResponse struct {
	...
	TasksAvarageCompletionTime *string  `json:"tasks_average_completion_time" example:"1m30s"`
}
...
// @Param 		from query int false "Начало промежутка рассмотрения статистики (включительно), формат: YYY-MM-DD"
// @Param 		to query int false "Конец промежутка рассмотрения статистики (не включительно), формат: YYY-MM-DD"
...
func getUserIDFromToQueryParams(r *http.Request) (*int, *time.Time, *time.Time, error) {
	...
	userID, err := core_http_request.GetIntPathValue(r, userIDQueryParamKey)
	if err != nil {
